...
class Generator(nn.Module):
    def __init__(self, num_points, num_features, hparams, init_weights=None):
        ...
        for idx in range(1, len(hparams.generator_filters)):
            layers.update(
                {
                    "gen_linear{}".format(idx + 1): nn.Linear(
                        in_features=hparams.generator_filters[idx - 1],
            ...
            layers.update(
                {"gen_relu{}".format(idx + 1): self.activation(**self.activ_args)}
            )

        # last layer
        layers.update(
            {
                "gen_linear{}".format(idx + 2): nn.Linear(
                    in_features=hparams.generator_filters[-1],
...
class Discriminator(nn.Module):
    def __init__(self, hparams, init_weights=None):
        ...
        for idx in range(1, len(hparams.discriminator_filters)):
            layers.update(
                {
                    "dis_linear{}".format(idx + 1): nn.Linear(
                        in_features=hparams.discriminator_filters[idx - 1],
            ...
            layers.update(
                {"dis_relu{}".format(idx + 1): self.activation(**self.activ_args)}
            )

        # final layer
        layers.update(
            {
                "dis_linear{}".format(idx + 2): nn.Linear(
                    in_features=hparams.discriminator_filters[-1],
                    out_features=1,
                    bias=self.use_bias,
                )
            }
...
class Encoder(nn.Module):
    def __init__(self, num_points, num_features, hparams, init_weights=None):
        ...
        for idx in range(1, len(hparams.encoder_filters) - 1):
            layers.update(
                {
                    "enc_conv{}".format(idx + 1): nn.Conv1d(
                        in_channels=hparams.encoder_filters[idx - 1],
            ...
            layers.update(
                {"enc_relu{}".format(idx + 1): self.activation(**self.activ_args)}
            )

        # final layer
        layers.update(
            {
                "enc_conv{}".format(idx + 2): nn.Conv1d(
                    in_channels=hparams.encoder_filters[-2],
        ...
        self.fc = nn.Sequential(
            nn.Linear(
                hparams.encoder_filters[-1], hparams.encoder_filters[-2], bias=True
            ),
...
class AAE3dModel(nn.Module):
    def __init__(self, num_points, num_features, hparams, devices, init_weights):
        ...
        self.encoder = Encoder(num_points, num_features, hparams, init_weights).to(
            devices[0]
        )
        self.generator = Generator(num_points, num_features, hparams, init_weights).to(
            devices[1]
        )
...
class AAE3d(object):
    ...
    def __init__(
        ...
        # model
        self.model = AAE3dModel(
            num_points, num_features, hparams, self.devices, init_weights
        )
        ...
        self.fixed_noise = torch.FloatTensor(self.batch_size, hparams.latent_dim, 1).to(
            self.devices[2]
        )
        self.fixed_noise.normal_(mean=self.noise_mu, std=self.noise_std)
        self.noise = torch.FloatTensor(self.batch_size, hparams.latent_dim).to(
            self.devices[2]
        )
        ...
        self.optimizer_d = get_optimizer(
            self.model.discriminator.parameters(), optimizer_hparams
        )
        self.optimizer_eg = get_optimizer(
            chain(self.model.encoder.parameters(), self.model.generator.parameters()),
            optimizer_hparams,
        )
    ...
    def _configure_device(self, gpu):
        ...
        if isinstance(gpu, tuple) and len(gpu) == 3:
            return (
                torch.device(f"cuda:{gpu[0]}"),
                torch.device(f"cuda:{gpu[1]}"),
                torch.device(f"cuda:{gpu[2]}"),
            )

        raise ValueError(
            "Specified GPU device is invalid. Should be int, 3-tuple or None."
        )
    ...
    def _loss_fnc_eg(self, data, rec_batch, fake_logit):
        ...
        loss = self.lambda_rec * torch.mean(
            self.rec_loss(rec_batch.permute(0, 2, 1), data.permute(0, 2, 1))
        )
    ...
    def train(self, train_loader, valid_loader, epochs=1, checkpoint="", callbacks=[]):
        ...
        if callbacks:
            ...
            logs = {
                ...
                "optimizer_eg": self.optimizer_eg,
            }
            if dist.is_initialized():
    ...
    def _train(self, train_loader, epoch, callbacks, logs):
        ...
        for batch_idx, token in enumerate(train_loader):
            ...
            if self.verbose and (self.comm_rank == 0):
                print(
                    "Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss_d: {:.6f}\tLoss_eg: {:.6f}\tTime: {:.3f}".format(
                        epoch,
        ...
        if self.verbose and (self.comm_rank == 0):
            print(
                "====> Epoch: {} Average loss_d: {:.4f} loss_eg: {:.4f}".format(
                    epoch, train_loss_d_ave, train_loss_eg_ave
                )
            )
