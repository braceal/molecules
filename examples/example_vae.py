...
from molecules.ml.unsupervised.vae import (
    VAE,
    SymmetricVAEHyperparams,
    ResnetVAEHyperparams,
)
...
@click.option(
    "-rn", "--rmsd_name", default="rmsd", help="Name of the RMSD data in the HDF5 file."
)
@click.option(
    "-fn",
    "--fnc_name",
...
@click.option(
    "-m", "--model_prefix", required=True, help="Model prefix for file naming."
)
@click.option(
    "-h", "--dim1", required=True, type=int, help="H of (H,W) shaped contact matrix"
)
@click.option(
    "-w", "--dim2", required=True, type=int, help="W of (H,W) shaped contact matrix"
)
@click.option(
    "-c",
    "--checkpoint",
...
@click.option(
    ...
    help="Format of contact map files. Options " "[full, sparse-concat, sparse-rowcol]",
)
@click.option("-E", "--encoder_gpu", default=None, type=int, help="Encoder GPU id")
@click.option("-D", "--decoder_gpu", default=None, type=int, help="Decoder GPU id")
@click.option(
...
def main(
    ...
    if tsne_interval < embed_interval:
        warnings.warn(
            "Found tsne_interval < embed_interval. Will result in "
    ...
    if distributed and dist.is_available():
        ...
        if local_rank is not None:
            comm_local_rank = local_rank  # noqa
        else:
            comm_local_rank = int(os.getenv("LOCAL_RANK", 0))  # noqa
    ...
    if comm_size > 1:
        if encoder_gpu == decoder_gpu:
            vae.model = DDP(
                vae.model, device_ids=[enc_device], output_device=enc_device
            )
        else:
